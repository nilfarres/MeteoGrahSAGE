...
# ----------------- MAPA DE TEMPERATURA -----------------
plt.figure(figsize=(8, 6))
sc = plt.scatter(df["lon"], df["lat"], c=df["Temp"], cmap="coolwarm", marker="o", edgecolor="black", s=50)
...
# ----------------- MAPA D’HUMITAT -----------------
plt.figure(figsize=(8, 6))
sc = plt.scatter(df["lon"], df["lat"], c=df["Humitat"], cmap="Blues", marker="o", edgecolor="black", s=50)
...
if df["Pluja"].max() > 0:
    plt.figure(figsize=(8, 6))
    sc = plt.scatter(df["lon"], df["lat"], c=df["Pluja"], cmap="Greens", marker="o", edgecolor="black", s=50)
...
plt.colorbar(sc, label="Força del vent (km/h)")
plt.xlabel("Longitud")
plt.ylabel("Latitud")
plt.title("Força del Vent a les Estacions Meteorològiques")
plt.grid(True)
plt.tight_layout()
plt.show()
